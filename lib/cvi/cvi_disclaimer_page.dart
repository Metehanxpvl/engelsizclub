...
import 'cvi_config_loader.dart';
import 'cvi_exercise_page.dart';
import '../l10n/l10n_text.dart';

/// Zorunlu yasal / tıbbi sorumluluk reddi — işaretlemeden başlanamaz.
class CviDisclaimerPage extends StatefulWidget {
  const CviDisclaimerPage({
    super.key,
    this.isGuest = false,
    this.onRequireLogin,
  });
  ...
  @override
  State<CviDisclaimerPage> createState() => _CviDisclaimerPageState();
}

class _CviDisclaimerPageState extends State<CviDisclaimerPage> {
  ...
  Future<void> _start() async {
    ...
    setState(() => _busy = true);
    try {
      final config = await CviConfigLoader.load();
      if (!mounted) return;
      if (config.steps.isEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: L10nText('Egzersiz yapılandırması yüklenemedi.')),
        );
        return;
      }
      await Navigator.of(context).pushReplacement(
        MaterialPageRoute<void>(
          builder: (_) => CviExercisePage(
            config: config,
                  ...
                  child: Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        L10nText(
                          'Bu araç yalnızca görsel egzersiz ve takip amaçlıdır. '
                          'Tıbbi tanı koymaz, tedavi önermez ve klinik değerlendirme yerine geçmez.',
                          style: GoogleFonts.nunito(
                            fontSize: 16,
                            height: 1.45,
                        ...
                        const _Bullet(
                          'Sonuçlar egzersiz performansı içindir; tıbbi karar vermek için kullanılamaz.',
                        ),
                        const _Bullet(
                          'Geliştiriciler ve yayıncı, kullanım nedeniyle doğabilecek tıbbi sonuçlardan sorumlu değildir.',
                        ),
                        const _Bullet(
                        ...
                        Container(
                          ...
                          child: L10nText(
                            'Bu uygulama tıbbi tanı koymaz. Sadece görsel egzersiz ve takip içindir.',
                            style: GoogleFonts.nunito(
                              fontSize: 15,
                              fontWeight: FontWeight.w800,
                ...
                child: FilledButton(
                  ...
                  style: FilledButton.styleFrom(
                    ...
                    disabledBackgroundColor:
                        CviColors.primary.withValues(alpha: 0.35),
